...
namespace Scene.ClassRoom {
    export class EditEquip implements Game.Scene.Scene {
        draw() {}
        update() {}
        ...
        constructor() {
            ...
            const charactorListBox = new Game.GUI.ListBox({
                ...
                drawItem: (left: number, top: number, width: number, height: number, index: number) => {
                    drawStatusSprite(charactors[index],
                        selectedCharactorIndex == index ? DrawMode.Selected : DrawMode.Normal,
                        left,
                        top,
                        width,
                        height,
            ...
            const itemListBox = new Game.GUI.ListBox({
                ...
                drawItem: (left: number, top: number, width: number, height: number, index: number) => {
                    ...
                    Game.getScreen().fillText(Data.Item.get(Data.SaveData.itemBox[itemLists[index]].id).name,
                        left + 3,
                        top + 3);
                }
            });
            ...
            statusViewBtn.draw = () => {
                drawStatusSprite(charactors[selectedCharactorIndex],
                    DrawMode.Normal,
                    statusViewBtn.left,
                    statusViewBtn.top,
                    statusViewBtn.width,
