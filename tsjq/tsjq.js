...
(function (Unit) {
    let BuffKind;
    (function (BuffKind) {
        BuffKind[BuffKind["AdditiveTurn"] = 0] = "AdditiveTurn";
        BuffKind[BuffKind["MPDecTime"] = 1] = "MPDecTime";
    })(BuffKind = Unit.BuffKind || (Unit.BuffKind = {}));
    function AdditiveTurn(interval) {
        return {
            availableTime: -1,
    ...
    Unit.AdditiveTurn = AdditiveTurn;
    function MPDecTime(interval) {
        return {
            availableTime: -1,
    ...
    Unit.MPDecTime = MPDecTime;
    function reduceEquips(equipData, pred, seed) {
        return equipData.reduce((s, [v, k]) => (v == null) ? s : pred(s, Data.Item.get(v.id)), seed);
    }
    function setupMemberStatus(id) {
        ...
        const ret = {
            ...
            additiveTurn: 0,
            mpDecTime: 10,
            buffs: reduceEquips(memberData.equips, (s, x) => { x.buffs(s); return s; }, []),
        };
        updateMemberStatus(ret);
        return ret;
    }
    function updateMemberStatus(memberStatus) {
            ...
            if (x.availableTime > 0 || x.availableTime == -1) {
                switch (x.kind) {
                    case BuffKind.AdditiveTurn:
                        memberStatus.additiveTurn += x.workdata.inverval;
                        break;
    ...
    Unit.updateMemberStatus = updateMemberStatus;
    /**
     * パーティ
     */
    class Party extends Unit.UnitBase {
    ...
    (function (Item) {
        ...
        const Datas = [
            ...
            },
            {
                ...
                atk: 0,
                def: 1,
                buffs: (datas) => datas.push(Unit.AdditiveTurn(3)),
                stackable: false
            },
            {
                id: 308,
                ...
                atk: 3,
                def: 0,
                buffs: (datas) => datas.push(Unit.MPDecTime(5)),
                stackable: false
            },
            /* Tool */
            {
            ...
            btnDungeon.click = (x, y) => {
                ...
                selected = () => {
                    ...
                    Game.getSceneManager().push(new Scene.Dungeon.Top({
                        player: new Unit.Party(Data.SaveData.findCharactorById(Data.SaveData.forwardCharactor), Data.SaveData.findCharactorById(Data.SaveData.backwardCharactor)),
                        floor: 1
                    }));
                };
            };
        ...
        class GameOver {
            constructor(opt) {
                ...
                this.update = Scene.waitFadeOut(fade, () => {
                    ...
                    this.update = Scene.waitTimeout(500, () => this.update = Scene.waitClick(() => {
                        Game.getSceneManager().pop();
                        Game.getSceneManager().push(new Scene.Title());
        ...
        class Top {
            constructor(param) {
                ...
                let monsters = map.rooms.splice(2).map((x) => {
                    ...
                    monster.dropRate = {
                        50: (x, y) => new GoldBug(x, y, ~~((9 + floor) * 0.1 * (Math.random() * 9 + 1))),
                        5: (x, y) => new ItemBug(x, y, [{ id: 1001, condition: "", count: 1 }]),
                    };
                    return monster;
                });
                // ドロップアイテム等の情報
            ...
            for (;;) {
                ...
                if (targetMonster !== -1) {
                    ...
                    if (context.player.additiveTurnValue >= 100) {
                        ...
                        turnStateStack.unshift(PlayerAction.call(this, turnStateStack, context), TurnEnd.call(this, turnStateStack, context));
                    }
                    else {
                    ...
                    if (context.player.additiveTurnValue >= 100) {
                        ...
                        context.tactics.monsters.fill(null);
                        // 移動実行のみ行う。
                        turnStateStack.unshift(Move.call(this, turnStateStack, context), TurnEnd.call(this, turnStateStack, context));
                    }
                    else {
    ...
    class TalkScene {
        constructor() {
            ...
            const fadein = () => {
                ...
                fade.startFadeIn();
                this.update = Scene.waitTimeout(1000, () => {
                    this.update = Scene.waitFadeIn(fade, () => {
                        caption.visible = true;
                        this.update = Scene.waitTimeout(500, () => talk(0));
                    });
                });
            };
            const talk = (index) => {
            ...
            const wait = () => this.update = Scene.waitTimeout(1000, () => {
        ...
        constructor() {
            ...
            const updateExit = () => {
                if (momyu > 0) {
                    ...
                    this.update = Scene.waitTimeout(1000, smashExit);
                }
                else {
