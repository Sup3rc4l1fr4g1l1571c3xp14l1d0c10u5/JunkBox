...
(function (Sound) {
    ...
    class SoundManager {
        ...
        loadSoundToChannel(file, channel) {
            return this._loadSound(file).then((audioBufferNode) => {
        ...
        loadSoundsToChannel(config) {
            return Promise.all(Object.keys(config)
                .map((x) => ~~x)
                .map((channel) => this._loadSound(config[channel]).then((audioBufferNode) => {
                this.channels[channel].audioBufferNode = audioBufferNode;
            }))).then(() => { });
        }
        ...
        stop() {
            ...
            for (let bufferid /* number */ in oldPlayingBufferSources) {
                if (oldPlayingBufferSources.hasOwnProperty(bufferid)) {
                    const s = oldPlayingBufferSources[bufferid].buffer;
                    if (s != null) {
                        s.stop(0);
                        s.disconnect();
        ...
        reset() {
            for (let i = 0; i < SoundManager.SOUND_CHANNEL_MAX; i++) {
    ...
    SoundManager.SOUND_CHANNEL_MAX = 36 * 36;
...
class Matrix {
    constructor(width, height, fill) {
        ...
        if (fill == undefined) {
            ...
            this._buffer = new Array(width * height).fill(fill);
        }
    }
    get width() {
        return this._width;
    ...
    draw(layerDrawHook) {
            ...
            for (let y = -1; y < yy; y++) {
                for (let x = -1; x < xx; x++) {
                    ...
                    if (this.layer[l].chip[chipid]) {
                        Game.getScreen().context.drawImage(Game.getScreen().texture("mapchip"), this.layer[l].chip[chipid].x, this.layer[l].chip[chipid].y, gridw, gridh, 0 + x * gridw + this.camera.chipOffX + gridw / 2, 0 + y * gridh + this.camera.chipOffY + gridh / 2, gridw, gridh);
                    }
                }
            }
            // レイヤー描画フック
        ...
        for (let y = -1; y < yy; y++) {
            for (let x = -1; x < xx; x++) {
                ...
                Game.getScreen().context.fillRect(0 + x * gridw + this.camera.chipOffX + gridw / 2, 0 + y * gridh + this.camera.chipOffY + gridh / 2, gridw, gridh);
            }
        }
    }
}
    ...
    Game.create({
        ...
        scene: {
            ...
            classroom: function* (data) {
                ...
                this.draw = () => {
                    ...
                    for (var y = 0; y < 5; y++) {
                        for (var x = 0; x < 6; x++) {
                            var id = y * 6 + x;
                            Game.getScreen().context.drawImage(Game.getScreen().texture("charactor"), 752 * (id % 2) +
            ...
            dungeon: function* (param) {
                ...
                const dungeon = DungeonGenerator.create(mapChipW, mapChipH, (x, y, v) => { mapchipsL1.value(x, y, v ? 0 : 1); });
                // 装飾
                ...
                this.draw = () => {
                    ...
                    map.draw((l, cameraLocalPx, cameraLocalPy) => {
                        if (l == 0) {
                            ...
                            Game.getScreen().context.ellipse(cameraLocalPx, cameraLocalPy + 7, 12, 3, 0, 0, Math.PI * 2);
                    ...
                    if (pad.isTouching) {
                        ...
                        Game.getScreen().context.beginPath();
                        Game.getScreen().context.ellipse(pad.x, pad.y, pad.radius * 1.2, pad.radius * 1.2, 0, 0, Math.PI * 2);
                        Game.getScreen().context.fill();
                        Game.getScreen().context.beginPath();
                        Game.getScreen().context.ellipse(pad.x + pad.cx, pad.y + pad.cy, pad.radius, pad.radius, 0, 0, Math.PI * 2);
                        Game.getScreen().context.fill();
                    }
                };
                this.update = (delta, ms) => {
