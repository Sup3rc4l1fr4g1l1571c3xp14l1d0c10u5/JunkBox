...
namespace MiniMAL {
    public static class Typing {
            ...
            switch (op) {
                case Expressions.BuiltinOp.Kind.Plus: {
                        if (args.Length == 2) {
                            return Tuple.Create(
                            (Type)new Type.TyInt(),
                            LinkedList.Create(args.Select(x => new TypeEquality(x, new Type.TyInt()))
                            .ToArray())
                            );
                        }
        ...
        private static Result eval_declEntry(Environment<Type> env, Declarations.Decls.DeclBase p) {
