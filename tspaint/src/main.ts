...
class ImageProcessing {
    ...
    private allocCachedTempOffScreen(width: number, height: number): Surface {
        const index : number = this.cachedSurface.findIndex(x => x.width == width && x.height == height);
        if (index != -1) {
    ...
    private freeCachedTempOffScreen(surface: Surface) {
    ...
    applyKernel(dst: Surface, src: Surface, { kernel = null, program = null }: { kernel: Float32Array, program: string }) {
        const gl = this.gl;
    ...
    copySurface(dst: Surface, src: Surface): void {
        if (dst != null) {
            const gl = dst.gl;
            gl.bindFramebuffer(gl.FRAMEBUFFER, src.framebuffer);
            gl.activeTexture(gl.TEXTURE0);
    ...
    drawLines(dst: Surface, { vertexes = null, size = null, color = null, antialiasSize = null }: { vertexes: Float32Array, size: number, color: [number, number, number, number], antialiasSize: number }): void {
        const gl = this.gl;

        const tmp: Surface = this.allocCachedTempOffScreen(dst.width, dst.height);
    ...
    fillRect(dst: Surface, { compositMode = CompositMode.MaxAlpha, start = null, end = null, color = null }: { compositMode: CompositMode, start: IPoint, end: IPoint, color: [number, number, number, number] }): void {
        const gl = this.gl;
    ...
    draw(dst: Surface, front: Surface, {compositMode = null, start = null, end, matrix = null}: { compositMode: CompositMode, start?: IPoint, end?: IPoint, matrix?: Matrix3 }) {
        const gl = this.gl;
    ...
    draw2(dst: Surface, back: Surface, front: Surface, {compositMode = null, start = null, end, matrix = null}: { compositMode: CompositMode, start?: IPoint, end?: IPoint, matrix?: Matrix3 }) {
        const gl = this.gl;
...
class SelectTool implements Tool {
    ...
    public draw(config: IPainterConfig, ip: ImageProcessing, overlay: Surface, imgData: Surface, finish: boolean): void {
...
module Painter {
    ...
    let updateRequest: { view: boolean, gui: boolean } = { view: false, gui: false };
    ...
    export function init(parentHtmlElement: HTMLElement, width: number, height: number) {
        ...
        tools.push(
            ...
            new SolidPen("Eraser", CompositMode.Erase),
            new RectanglePen("Rect", CompositMode.Normal),
            new FloodFill("Fill", CompositMode.Normal),
            new SelectTool("SelectBox", CompositMode.Normal)
        );
    ...
    export function update({ view = false, gui = false }: { view?: boolean, gui?: boolean }) {
