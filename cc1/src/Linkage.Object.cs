...
namespace AnsiCParser.Linkage {
    ...
    public class Object {
        ...
        private Object(string ident, CType type, Kind linkage, bool unique = false) {
            Id = _idCnt++;
            Ident = ident;
            Type = type;
            Linkage = linkage;
            LinkageId = Linkage == Kind.ExternalLinkage ? $"_{ident}" : Linkage == Kind.NoLinkage && unique ? $"{ident}.{_idCnt}" : $"{ident}";
        }

        public static Object Create(Declaration declaration, Kind linkage) {
            return new Object(declaration.Ident, declaration.Type, linkage);
        }

        public static Object CreateUnique(Declaration definition, Kind linkage) {
            return new Object(definition.Ident, definition.Type, linkage, true);
        }
