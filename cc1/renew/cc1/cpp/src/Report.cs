...
namespace CSCPP {
    ...
    public static class Reporting {
        ...
        public static class TraceCompileSwitch {
            [Worksheet(name: "コンパイルスイッチ情報")]
            public class Info {
                [WorksheetMember(name: "ID", order: 0)]
                public int Id { get; }

                [WorksheetMember(name: "種別", order: 6)]
                public string Kind { get; }

                public Position Position { get; }

                [WorksheetMember(name: "宣言ファイル", order: 2)]
                public string FileName { get { return Position.FileName; } }

                [WorksheetMember(name: "行", order: 2)]
                public long FileLine { get { return Position.Line; } }

                [WorksheetMember(name: "列", order: 3)]
                public int FileColumn { get { return Position.Column; } }

                [WorksheetMember(name: "式", order: 7)]
                public string Expr { get; }
                public bool Active { get; }
                [WorksheetMember(name: "ネスト", order: 4)]
                public int Depth { get; }
                [WorksheetMember(name: "グループ", order: 5)]
